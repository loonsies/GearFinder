function DrawGUI(imgui, uiConfig, ffi, gearGlobals, allStats, rtGlobals, memAddress)
    ...
    if uiConfig.is_open[1] then
        if imgui.Begin(("GearFinder-v%s##Main_Window"):fmt(addon.version), uiConfig.is_open, windowFlag or ImGuiWindowFlags_AlwaysAutoResize) then
                ...
                if (eit.item.id) then
                    ...
                    if (resItem) then
                        ...
                        if (imgui.ImageButton(tonumber(ffi.cast("uint32_t", eit.texture)), { 40, 42 })) then
                            ...
                            uiConfig.select_pos_offset = { cursorPosX - uiConfig.main_pos[1] + ((xOffset - 1) * 56),
                                cursorPosY - uiConfig.main_pos[2] };
                        ...
                        if (imgui.IsItemHovered()) then
                            if (gearGlobals.hoveredTooltip.item ~= eit.item) then
                                ...
                                gearGlobals.hoveredTooltip.text = GetItemTooltip(eit.item, gearGlobals, allStats,
                                    uiConfig);
                            end
                            imgui.BeginTooltip();
            ...
            -- end
            imgui.BeginChild(addon.name .. "_search_filters", { 225, -1 }, ImGuiChildFlags_Borders);
            ...
            imgui.BeginChild(addon.name .. "_stat_overview", { -1, -1 }, ImGuiChildFlags_Borders);
                    ...
                    for _, sIx in ipairs(v.stats) do
                        if (gearGlobals.workingStats[sIx]) then
                            ...
                            if (sIx == "wtype") then
                                ...
                                if (sdata.min ~= 0) then
                                    outS = ("%s: %s%d ~ %d"):format(useName, oper, stTotal, (stTotal - sdata.min) + sdata
                                        .max);
                                else
    ...
    if (uiConfig.selecting_gear[1]) then
        ...
        if (imgui.Begin("##slot_select" .. addon.name, uiConfig.selecting_gear, ImGuiWindowFlags_NoTitleBar or ImGuiWindowFlags_NoSavedSettings)) then
            ...
            imgui.BeginChild(addon.name .. "_selectable_gear_list", { -1, -1 }, ImGuiChildFlags_Borders);
